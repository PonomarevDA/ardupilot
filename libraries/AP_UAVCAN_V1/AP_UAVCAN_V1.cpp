...
///< Base must be aligned for O1HEAP_ALIGNMENT byes. Othwerwise o1heapInit may fails!
...
void AP_UAVCAN_V1::init(uint8_t driver_index, bool enable_filters)
{
    ...
    if (NULL == my_allocator) {
        gcs().send_text(MAV_SEVERITY_WARNING, "v1: o1heapInit() failed");
        return;
    }
...
bool AP_UAVCAN_V1::add_interface(AP_HAL::CANIface* new_can_iface)
{
    ...
    if (_can_iface == nullptr) {
        gcs().send_text(MAV_SEVERITY_WARNING, "v1: CAN driver not found");
        return false;
    }

    if (!_can_iface->is_initialized()) {
        gcs().send_text(MAV_SEVERITY_WARNING, "v1: Driver not initialized");
        return false;
    }
...
void AP_UAVCAN_V1::spinReceive()
{
    ...
    for (uint_fast8_t frame_idx = 0; frame_idx < CAN_RECEIVE_FIFO_SIZE; frame_idx++) {
        ...
        if (result < 0) {
            gcs().send_text(MAV_SEVERITY_WARNING, "v1: recv err: %d", result);
